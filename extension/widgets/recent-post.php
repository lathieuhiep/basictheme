...
class Wp_Recent_Posts_Thumbs_Widget extends WP_Widget {
    ...
    public function __construct() {
        ...
        $basictheme_recent_posts_thumbs_widget_ops = array(
            'classname'     =>  'recent_posts_thumbs_widget',
            'description'   =>  'Recent Posts Thumbs Widget',
        );

        parent::__construct( 'recent_posts_thumbs_widget', 'Recent Posts Thumbs Widget', $basictheme_recent_posts_thumbs_widget_ops );
    ...
    public function widget( $args, $instance ) {
        ...
        $basictheme_recent_posts_thumbs_limit       =   isset( $instance['number'] ) ? $instance['number'] : 5;
        $basictheme_recent_posts_thumbs_type_post   =   !empty( $instance['type_post'] ) ? $instance['type_post'] : 'list';
        $basictheme_recent_posts_thumbs_select_cat  =   !empty( $instance['select_cat'] ) ? $instance['select_cat'] : array( '0' );
        $basictheme_recent_posts_thumbs_hide_date   =   isset( $instance['hide_date'] ) ? $instance['hide_date'] : false;

        $basictheme_recent_posts_thumbs_paged = 1;

        if ( in_array( 0, $basictheme_recent_posts_thumbs_select_cat ) ) :

            $basictheme_recent_posts_thumbs_args    =   array(
                'post_type'             =>  'post',
                'post_status'           =>  'publish',
                'posts_per_page'        =>  $basictheme_recent_posts_thumbs_limit,
                'order'                 =>  $instance['order'],
                'orderby'               =>  $instance['order_by'],
                'ignore_sticky_posts'   =>  1,
                'paged'                 =>  $basictheme_recent_posts_thumbs_paged,
            );
            ...
            $basictheme_recent_posts_thumbs_args    =   array(
                'post_type'             =>  'post',
                'post_status'           =>  'publish',
                'cat'                   =>  $basictheme_recent_posts_thumbs_select_cat,
                'posts_per_page'        =>  $basictheme_recent_posts_thumbs_limit,
                'order'                 =>  $instance['order'],
                'orderby'               =>  $instance['order_by'],
                'ignore_sticky_posts'   =>  1,
        ...
        $basictheme_recent_posts_thumbs_query   =   new WP_Query( $basictheme_recent_posts_thumbs_args );

        if ( $basictheme_recent_posts_thumbs_type_post == 'list' ) :
            $basictheme_recent_post_type = ' wp_recent_posts_list';
        elseif ( $basictheme_recent_posts_thumbs_type_post == 'fist_large_post' ) :
            $basictheme_recent_post_type = ' wp_recent_posts_fist_large';
        else:
            $basictheme_recent_post_type = ' wp_recent_posts_grid';
        endif;

        if ( $basictheme_recent_posts_thumbs_query->have_posts() ) :


        ?>

            <div class="wp_recent_posts_thumbs_widget_warp">
                <div class="wp_recent_posts_thumbs_widget<?php echo esc_attr( $basictheme_recent_post_type ) ?>">

                    <?php
                    while ( $basictheme_recent_posts_thumbs_query->have_posts() ) :
                        $basictheme_recent_posts_thumbs_query->the_post();
                    ?>
                        ...
                        <div class="wp_recent_posts_thumbs_item">
                            ...
                            <div class="wp_recent_posts_thumbs_item--content">
                                ...
                                <?php if ( $basictheme_recent_posts_thumbs_hide_date == false ) : ?>
                                ...
                                <?php if ( $basictheme_recent_post_type == 'fist_large_post' && $basictheme_recent_posts_thumbs_query->current_post == 0 ) : ?>
