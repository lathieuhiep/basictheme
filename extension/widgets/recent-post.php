...
class basictheme_recent_posts_widget extends WP_Widget {
    ...
    public function __construct() {
        ...
        $basictheme_recent_posts_widget_ops = array(
            'classname'     =>  'recent_posts_thumbs_widget',
            'description'   =>  'Recent Posts Thumbs Widget',
        );

        parent::__construct( 'basictheme_recent_posts_thumbs_widget', 'Basic Theme: Recent Posts Thumbs Widget', $basictheme_recent_posts_widget_ops );
    ...
    public function widget( $args, $instance ) {
        ...
        $basictheme_recent_posts_limit       =   isset( $instance['number'] ) ? $instance['number'] : 5;
        $basictheme_recent_posts_type_post   =   !empty( $instance['type_post'] ) ? $instance['type_post'] : 'list';
        $basictheme_recent_posts_select_cat  =   !empty( $instance['select_cat'] ) ? $instance['select_cat'] : array( '0' );
        $basictheme_recent_posts_hide_date   =   isset( $instance['hide_date'] ) ? $instance['hide_date'] : false;

        $basictheme_recent_posts_paged = 1;

        if ( in_array( 0, $basictheme_recent_posts_select_cat ) ) :

            $basictheme_recent_posts_args    =   array(
                'post_type'             =>  'post',
                'post_status'           =>  'publish',
                'posts_per_page'        =>  $basictheme_recent_posts_limit,
                'order'                 =>  $instance['order'],
                'orderby'               =>  $instance['order_by'],
                'ignore_sticky_posts'   =>  1,
                'paged'                 =>  $basictheme_recent_posts_paged,
            );
            ...
            $basictheme_recent_posts_args    =   array(
                'post_type'             =>  'post',
                'post_status'           =>  'publish',
                'cat'                   =>  $basictheme_recent_posts_select_cat,
                'posts_per_page'        =>  $basictheme_recent_posts_limit,
                'order'                 =>  $instance['order'],
                'orderby'               =>  $instance['order_by'],
                'ignore_sticky_posts'   =>  1,
        ...
        $basictheme_recent_posts_query   =   new WP_Query( $basictheme_recent_posts_args );

        if ( $basictheme_recent_posts_type_post == 'list' ) :
            $basictheme_recent_post_type = ' recent_posts_thumbs_widget_list';
        elseif ( $basictheme_recent_posts_type_post == 'fist_large_post' ) :
            $basictheme_recent_post_type = ' recent_posts_thumbs_widget_fist_large';
        else:
            $basictheme_recent_post_type = ' recent_posts_thumbs_widget_grid';
        endif;

        if ( $basictheme_recent_posts_query->have_posts() ) :

        ?>

            <div class="recent_posts_thumbs_widget_warp">
                <div class="recent_posts_thumbs_widget<?php echo esc_attr( $basictheme_recent_post_type ) ?>">

                    <?php
                    while ( $basictheme_recent_posts_query->have_posts() ) :
                        $basictheme_recent_posts_query->the_post();
                    ?>
                        ...
                        <div class="recent_posts_thumbs_widget_item">
                            ...
                            <div class="recent_posts_thumbs_widget_item--content">
                                ...
                                <?php if ( $basictheme_recent_posts_hide_date == false ) : ?>
                                ...
                                <?php if ( $basictheme_recent_post_type == 'fist_large_post' && $basictheme_recent_posts_query->current_post == 0 ) : ?>
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>">
                <?php esc_attr_e( 'Title:', 'basictheme' ); ?>
            </label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'type_post' ) ); ?>">
                <?php esc_attr_e( 'Type post:', 'basictheme' ); ?>
            </label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>">
                <?php esc_attr_e( 'Select Category:', 'basictheme' ); ?>
            </label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>">
                <?php esc_attr_e( 'Order:', 'basictheme' ); ?>
            </label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>">
                <?php esc_attr_e( 'Number of posts to show:', 'basictheme' ); ?>
            </label>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id( 'hide_date' ); ?>">
                <?php esc_html_e( 'Hide post date?', 'basictheme' ); ?>
            </label>
        </p>
        <!-- End check hide date -->
...
function basictheme_recent_posts_register_widget() {
    register_widget( 'basictheme_recent_posts_widget' );
}
add_action( 'widgets_init', 'basictheme_recent_posts_register_widget' );